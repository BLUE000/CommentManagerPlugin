...
| **UT-PLG-02** | メタ情報メソッド | `pluginId()`, `pluginName()`, `defaultAssets()` 等を呼ぶ。 | 1. IDとして `"com.blue000.twitchchannelmanagementtool.CommentManagerPlugin"` が返ること。<br>2. デフォルトアセットとして `comment.html` および `ranking.html` のバイナリデータがマップに格納されていること。 |
