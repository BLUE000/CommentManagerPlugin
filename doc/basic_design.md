...
### 3.1. 左ペイン: コメントリスト表示部
- リスト形式 (`QListView` または `QListWidget`) を使用し、コメントを上から下へ追加します。
- 各行のカスタムペイント (`QStyledItemDelegate`) またはリッチテキストサポートにより、ユーザー名とバッジ、コメント本文を適切に色分け表示します。
- **除外ユーザー処理**: `ConfigManager` の除外ユーザー一覧に登録されているユーザーからのコメントは、リスト追加時に破棄（非表示）されます。
